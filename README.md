...
CopyQR first reads the selection directly from the foreground app without changing the clipboard. If the app does not expose its selection, CopyQR performs a normal copy and leaves that text on the clipboard. Clipboard managers such as Maccy will record the fallback copy.
